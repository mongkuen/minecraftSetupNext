...
PSInput.rainpos = VSInput.position.xyz;

#ifdef ALPHA_TEST
if(PSInput.color.r+PSInput.color.g > PSInput.color.r+PSInput.color.r)
{
PSInput.position += cos( length( abs( VSInput.position.xyz - 8.0 ) * 5.0  ) + TIME * 4.0 ) * 0.01;
}
#endif
...
if( PSInput.color.b > PSInput.color. r * 1.1 && PSInput.color.b>= PSInput.color.g )
{
PSInput.position.y += cos( length( abs( VSInput.position.xyz - 8.0 ) * 10.0  ) + TIME * 4.0 ) * 0.03;
}
